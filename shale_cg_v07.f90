...
!
! V0.7 - parallelised with OpenMP and MPI, blockwise build of A
!
! Tobias Wittwer, 2006
program shale
    implicit none
    ...
    ! arrays
    real*8, allocatable :: a_block(:,:)                    ! design matrix block
    ...
    real*8, allocatable :: b2(:,:),help(:)

    ! other
    integer*4 :: i, it, j, k
    real*8 :: value, theta, latold, factor
    real*8 :: alpha, beta, maxx,br, norm0, gamma
    ...
    xold=x
    xt=x
    !$OMP PARALLEL PRIVATE(a_block,latold,pnm,threadnum,idx,i,help)
    latold = 1.6d0
    allocate(pnm(0:nmax,0:nmax))
    allocate(a_block(u,blocksize),help(blocksize))
    ...
    do j=myrank+1,numblocks,nprocs
        ...
        call dgemv('N',u,blocksize,1.0d0,a_block,u,help,1,1.0d0,b2(1,threadnum),1)
        !call daxpy(u,y(i),a_row,1,b2(1,threadnum),1)
    end do
    !$OMP END DO
    deallocate(pnm,a_block,help)
    !$OMP END PARALLEL
    do j=2,numthreads
        b2(:,1) = b2(:,1)+b2(:,j)
    end do
    ...
    do it=1,u
        ...
        a(:) = 0.0d0
        b2(:,:) = 0.0d0
        !$OMP PARALLEL PRIVATE(a_block,latold,pnm,help,threadnum,idx,i)
        latold = 1.6d0
        allocate(pnm(0:nmax,0:nmax))
        allocate(a_block(u,blocksize),help(blocksize))
        threadnum = omp_get_thread_num()+1
        ...
        do j=myrank+1,numblocks,nprocs
            ...
            call dgemv('T',u,blocksize,1.0d0,a_block,u,p,1,0.0d0,help,1)
            call dgemv('N',u,blocksize,1.0d0,a_block,u,help,1,1.0d0,b2(1,threadnum),1)
