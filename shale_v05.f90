...
!
! V0.5 - parallelised with OpenMP and ScaLAPACK
!
! Tobias Wittwer, 2006
program shale
    implicit none
    ...
    ! ScaLAPACK
    integer*4 :: iam, nprocs, ictxt, idx
    ...
    ! allocate arrays
    allocate(A(ra,ca),N(rn,cn),y(ry),b(rb))
    ...
    ! build design matrix
    time = omp_get_wtime()
    !$OMP PARALLEL PRIVATE(latold,pnm,idx)
    ...
    do i=1,ra
        ...
        call build_a_line(A,i,nobs,ra,nmax,u,long(idx),lat(idx),rad(idx),latold,pnm,rearth)
        y(i)=obs(idx)
        latold = lat(idx)
    end do
    !$OMP END DO
...
subroutine build_a_line(a,i,nobs,ra,nmax,u,long,lat,rad,latold,p,rearth)
    ...
    real*8 :: a(ra,u), long, lat, rad, latold
...
subroutine calc_sizes_descinit(blocksize,ictxt,myrow,mycol,nprow,npcol,m,n,desc,r,c)
    ...
    implicit none
    integer*4 :: m,n,desc(9),info,r,c,ictxt
    ...
    call descinit(desc,m,n,blocksize,blocksize,0,0,ictxt,r,info)
