...
subroutine build_precon(long,lat,rad,N,nmax,size_nbd,nobs,u,rearth)
    ...
    real*8, allocatable :: a_row(:), work(:), pnm(:,:), N2(:,:)
    ...
    !$OMP PARALLEL PRIVATE(latold,a_row,pnm,idx,offset,m,j,k,threadnum)
    allocate(pnm(0:nmax,0:nmax))
    allocate(a_row(u))
    latold=1.6d0
    ...
    do i=1,nobs
        ...
        do m=1,nmax
            ...
            do j=1,nmax-m+1
                do k=1,nmax-m+1
                    N2(idx,threadnum) = N2(idx,threadnum) + a_row(j+offset)*a_row(k+offset)
                    idx = idx+1
                end do
            end do
